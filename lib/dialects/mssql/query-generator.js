...
var QueryGenerator = {
  ...
  versionQuery: function() {
    // MSSQL versions aren't valid with semver
    var sql = [
      "DECLARE @version NVARCHAR(16) = CONVERT(NVARCHAR, SERVERPROPERTY('productversion'));",
      "SELECT",
        "LEFT(@version, LEN(@version) - CHARINDEX('.', REVERSE(@version)))  AS 'version'"
    ].join(' ');
  ...
  selectQuery: function (tableName, options, model) {
    var query = AbstractQueryGenerator.selectQuery.call(this, tableName, options, model);
    
    // MSSQL 2005/2008 limit/offset
    if (this.sequelize.options.databaseVersion !== 0 && semver.lt(this.sequelize.options.databaseVersion, '11.0.0')) {
      if (options.offset || options.limit) {
        var offset = options.offset || 0
          , limit = options.limit
          , pattern = /^(SELECT (.+))(ORDER BY .+)?;$/g
          , matches = pattern.exec(query);
          
        var order = matches[3] ? matches[3] : 'ORDER BY (SELECT 0)';
  ...
  addLimitAndOffset: function(options, model) {
    ...
    // MSSQL 2012+ limit/offset
    if (this.sequelize.options.databaseVersion !== 0 && semver.gte(this.sequelize.options.databaseVersion, '11.0.0')) {
